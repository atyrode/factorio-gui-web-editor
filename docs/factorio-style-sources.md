...
## Reusable Frame Edge Shader Plan

Issue: <https://github.com/atyrode/factorio-gui-web-editor/issues/20>

The next implementation attempt should not start by styling child Frames
directly. First extract the visual logic already working on the top-level Window
edge into a renderer-only edge shader that can paint either the outer side of a
box or the inner side of a box. The top-level Window remains the source of truth:
its current top glint, bottom dark edge, side ramps, sizes, and colors must not
change except as part of a no-visual-diff refactor that makes those bands
reusable.

The desired inner-frame effect is the same hollow-square problem as a 9-slice
panel: the outer shell already paints the outer edge of the hollow square; the
child/content boundary needs the same edge language applied to the inner edge.
This should become an explicit small vocabulary, not ad hoc shadows:

- edge side: top, bottom, left, right;
- edge orientation: outer or inner;
- edge source: the existing Window edge bands and color stops;
- join behavior: square/stepped joins unless a captured Factorio reference proves
  a miter;
- review state: shadow-visible and shadow-disabled screenshots from the same
  browser state after every visual edit.

Acceptance for the generic work:

- A documented mapping from current `.fx-gui-window__edge--*` CSS to reusable
  edge shader inputs exists before child Frame styling changes.
- Applying the shader back to the current Window produces fresh screenshots with
  no visible regression versus the current accepted outer shell.
- The child/body inner boundary uses the same shader inputs with inner
  orientation, not copied colors or one-off `box-shadow` values.
- Issue 20 receives checklist updates and fresh screenshots after each visual
  attempt; stale screenshots are invalid evidence.

Implemented mapping:

| Existing source | Shader orientation | Side | Source band | Direction | Join |
| --- | --- | --- | --- | --- | --- |
| `.fx-gui-window__edge--top` | outer | top | `top-glint` | `180deg` | Window miter clip, unchanged |
...
The generic shader is renderer chrome, but it is deliberately tied to the same
