...
The app is intended to be publicly reachable. It has no application login,
app-side access logging, analytics, or telemetry layer. The edge proxy examples
are plain public reverse-proxy routes.

Keep any future private credentials out of this repository. Use ignored local
environment files or the deployment platform's secret store only when a future
feature genuinely needs secrets.
...
checkout. The override sets `LABTORIO_HMR_HOST` to `labtorio.tyrode.dev`,
`LABTORIO_HMR_CLIENT_PORT` to `443`, and `LABTORIO_HMR_PATH` to `/@vite-hmr`, so
browser HMR uses the public HTTPS route.

Stop using the override and rebuild the static container before treating the
service as production again:
