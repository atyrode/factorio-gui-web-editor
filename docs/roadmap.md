...
[scriptable-api.md](scriptable-api.md). It exposes local structured commands
for Window creation, atom insertion/move/resize, Label captions, Lua variable
names, design import/export, Lua export, mod zip export, validation diagnostics,
and a Node runner for headless command batches. The browser facade is
`window.labtorioEditorApi`; it is a local page API, not a network service.
...
- expose constrained model operations to local scripts and test harnesses;
